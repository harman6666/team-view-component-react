...
import NativeSelectBox from '../NativeSelectBox';
...
export default class NativeTeamView extends React.Component {
  ...
  render() {
    ...
    return (
      <div className="team-view-wrapper container">
          ...
          <div className="native-view team-view pl-3 pr-3 col-12 col-md-10 col-lg-8 offset-md-1 offset-lg-2">
            <TeamViewHeader/>
            <ul className="row team-list ml-1 mr-1" ref={ref=> this.teamList = ref}> 
              <li className="col-12 col-md-6 team-item add-client-wrapper pl-2">
                ...
                <div className="select-box d-none" ref={ref => this.selectBox = ref}>
                  <NativeSelectBox onTaskSubmit={this.handleSubmit} fullTeam={this.state.fullTeamData} />
                </div>
              </li>
              {listNodes}
          </ul>
          <div className="show-all-btn d-none row" ref={ref=> this.showAllBtn = ref} onClick={this.showAll}>{teamViewLabels.showAllLabel}<i className="down ml-2 mb-1"></i></div>
          </div>
      </div>
    );
  }
